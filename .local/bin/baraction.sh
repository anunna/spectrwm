...
while :; do
 	...
 	eval $(cat /sys/class/power_supply/BAT0/status | awk '/charging state/ {printf "BAT_CHGSTATE=%s;", $3}; /remaining capacity/ {printf "BAT_REMNG=%s;",$3}; /present rate/ {printf "BAT_RATE=%s;",$3};' -)
 	eval $(cat /sys/class/power_supply/BAT0/uevent | awk '/present/ {printf "BAT_PRESENT=%s;", $2}; /last full capacity/ {printf "BAT_LASTFULL=%s;",$4};' -)
	...
	WLAN_ESSID=$(iwconfig wlp2s0 | awk -F "\"" '/wlp2s0/ { print $2 }')
